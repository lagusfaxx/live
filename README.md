# Live stack para app (Coolify + LiveKit + Token API)

Repositorio listo para desplegar en **Coolify** como app de tipo **Docker Compose**.

Incluye:
- `docker-compose.yml`
- `livekit/livekit.yaml` (SFU + TURN/STUN integrado)
...
- **LiveKit** = SFU WebRTC + TURN/STUN integrado para conectividad en redes restrictivas.
...
3. Puertos abiertos en firewall/proveedor:
   - `80/tcp`, `443/tcp`
   - `7880/tcp`, `7881/tcp`
   - `3478/tcp`, `3478/udp` (TURN/STUN integrado en LiveKit)
   - `5349/tcp`, `5349/udp` (TURN TLS integrado en LiveKit)
   - `50000-50100/udp`
...
2. Completa TODOS los placeholders:
   ...
   - Genera secretos largos para `LIVEKIT_API_SECRET` y `SHARED_BEARER_TOKEN`
...
**IMPORTANTE: NO sobreescribas los servidores ICE/TURN.** LiveKit envía su
propia configuración TURN a los clientes durante el signaling. Pasar
`rtcConfig: { iceServers: [...] }` en `room.connect()` reemplaza los servidores
TURN del servidor y rompe la conectividad.

```ts
// ✅ Correcto — LiveKit usa su TURN integrado automáticamente
await room.connect(tokenRes.url, tokenRes.token);

// ❌ Incorrecto — sobreescribe TURN del servidor con servidores externos
await room.connect(tokenRes.url, tokenRes.token, {
...
- [ ] El cliente **no** pasa `rtcConfig.iceServers` en `room.connect()` (LiveKit provee TURN automáticamente).
...
- LiveKit incluye TURN/STUN integrado (`turn.enabled: true` en `livekit.yaml`). No se necesita coturn externo; LiveKit genera las credenciales TURN y las envía a los clientes durante el signaling.
- El directorio `turn/` se conserva como referencia pero no se usa en producción.
